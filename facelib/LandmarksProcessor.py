...
from facelib.fs_align import AlignedFace
...
FaceType_to_padding_remove_align = {
    FaceType.HALF:          (0.0, 0.0, False),
    FaceType.MID_FULL:      (0.0675, 0.0, False),
    FaceType.FULL:          (0.2109375, 0.0, False),
    FaceType.FULL_NO_ALIGN: (0.2109375, 0.0, True),
    FaceType.WHOLE_FACE:    (0.40, 0.07, False),
    FaceType.BIG_FACE:      (0.50, 0.20, False),
    FaceType.SMALL_HEAD:    (0.55, 0.28, False),
    FaceType.HEAD:          (0.70, 0.50, False),
    FaceType.HEAD_NO_ALIGN: (0.70, 0.0, True),
}
...
def get_transform_mat (image_landmarks, output_size, face_type, scale=1.0):
    ...
    # if using a FS face-type, shunt to FS alignment algo
    if face_type in (FaceType.FS_FACE, FaceType.FS_HEAD):
        if face_type == FaceType.FS_FACE:
            fs_align = AlignedFace(image_landmarks, size=output_size, centering="face")
        elif face_type == FaceType.FS_HEAD:
            fs_align = AlignedFace(image_landmarks, size=output_size, centering="head")
        mat = fs_align.matrix * (output_size - 2 * fs_align.padding)
        mat[:, 2] += fs_align.padding
        return mat

    # FFHQ alignment calculation
    if face_type == FaceType.FFHQ:
    ...
    # calcs for DFL face-types below \/
    ###################################
    ...
    padding, v_offset, remove_align = FaceType_to_padding_remove_align.get(face_type, 0.0)
    mod = (1.0 / scale)* ( npla.norm(g_p[0]-g_p[2])*(padding*np.sqrt(2.0) + 0.5) )

    if face_type in (FaceType.WHOLE_FACE, FaceType.BIG_FACE):
        # adjust vertical offset for BIG_FACE, 20% below in order to cover more hair
        vec = (g_p[0]-g_p[3]).astype(np.float32)
        vec_len = npla.norm(vec)
        vec /= vec_len
        g_c += vec * vec_len * v_offset

    elif face_type in (FaceType.SMALL_HEAD, FaceType.HEAD):
        mat = umeyama( np.concatenate ( [ image_landmarks[17:49] , image_landmarks[54:55] ] ) , landmarks_2D_new, True)[0:2]
...
def get_image_eye_mask (image_shape, image_landmarks, soft=True):
...
def estimate_pitch_yaw_roll(aligned_landmarks, size=256):
    ...
    # S3FD's jaw landmarks are not suitable for a 3d projection - exclude them
    obj_pts = np.ascontiguousarray(landmarks_68_3D[17:]).astype(np.float64)
    img_pts = np.ascontiguousarray(aligned_landmarks[17:]).astype(np.float64)

    # get rotation and translation vector estimate
    #TODO: look at using translation and z rotation for alignment instead of umeyama
    _, rot_vec, trans_vec = cv2.solvePnP(obj_pts, img_pts, camera_matrix, None, flags=cv2.SOLVEPNP_ITERATIVE)

    # use SciPy's convenient Rotation object to skip matrix step
    r = R.from_rotvec(rot_vec.reshape((3,)))
    
    # DFL was decomposing rotations into XYZ order
    # YXZ seems to avoid gimbal lock better
