...
class DFLPNG(object):
        ...
        try:
            ...
            if inst.dfl_dict is not None:
                if 'face_type' not in inst.dfl_dict:
                    inst.dfl_dict['face_type'] = 'whole_face' #unclear why we need a default, but let's not depend on dfl module for it
    ...
    def embed_and_set(self, filename,   face_type=None,
        ...
        if cfg is None: cfg = self.get_cfg()
        if model_data is None: model_data = self.get_model_data()
    ...
    def get_image_to_face_mat(self):
        ...
        if mat is not None:
            return np.array (mat)
        return None
    def get_pitch_yaw_roll(self): return self.dfl_dict['pitch_yaw_roll']
    def get_eyebrows_expand_mod(self): return self.dfl_dict.get ('eyebrows_expand_mod', None)
    def get_cfg(self): return self.dfl_dict['cfg']
    def get_model_data(self): return self.dfl_dict['model_data']

    def get_seg_ie_polys(self): return None
    def get_xseg_mask(self): return None
